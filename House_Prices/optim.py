...
def objective(trial, base_cfg, device, X, y, search_output_dir):
    cfg = OmegaConf.merge(base_cfg, {})

    cfg.model.params.hidden_dims = trial.suggest_categorical(
        "hidden_dims", [[64, 32], [32, 16], [128, 64, 32], [64], [16], [8]]
    )
...
def main(config_path="config.yaml", n_trials=30):
    ...
    study = optuna.create_study(direction="minimize",sampler=optuna.samplers.TPESampler(seed=base_cfg.general.seed))
    study.optimize(lambda trial: objective(trial, base_cfg, device, X, y, search_output_dir), n_trials=n_trials)
